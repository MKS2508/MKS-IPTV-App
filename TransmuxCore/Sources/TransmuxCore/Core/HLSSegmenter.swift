...
public class HLSSegmenter {
    ...
    public init(fmp4Path: String, playlistPath: String, initSegmentSize: Int64, duration: Double) {
        ...
        TransmuxLog.segmenter("Created: fmp4=\(fmp4Path), playlist=\(playlistPath), initSize=\(initSegmentSize), duration=\(String(format: "%.1f", duration))s, virtualSegments=\(totalSegmentCount)")
    }
    ...
    public func start() {
        ...
        timer.resume()
        scanTimer = timer
        TransmuxLog.segmenter("Started scanning (timescale=\(timescale))")
    }
    ...
    private func parseTimescaleFromMoov() {
        ...
        while offset < fileSize {
            ...
            if boxType == "moov" {
                if let ts = findTimescaleInMoov(handle: handle, containerStart: offset + 8, containerEnd: offset + UInt64(boxSize)) {
                    timescale = ts
                    TransmuxLog.segmenter("Parsed timescale from moov/trak/mdia/mdhd: \(ts)")
                    return
                }
            }
        ...
        timescale = 90000
        TransmuxLog.segmenter("WARNING: Could not parse timescale from moov, using default \(timescale)", level: .warn)
    }
    ...
    public func latestBufferedSourceTime() -> Double {
        return scanQueue.sync {
            ...
            // This is more conservative but prevents serving incomplete data
